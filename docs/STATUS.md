...
Atualizado em: **2026-09-03**. Fonte única do índice Fatia ↔ SPEC. Estado remoto conferido nesta revisão: o MVP-009 foi **finalizado e o épico #100 fechado** pelo PI. As **cinco fatias do MVP-025 (#238 a #242) foram entregues** — a jornada com etapa derivada, o prompt/refinamento/brief por IA, o PRD/Landscape/Convention gerados com origem por afirmação, a arquitetura com âncora no protótipo e o roadmap/MVPs/SPEC gerados contra o validador de DAG. **O épico #237 fica com todas as filhas em `proplan:done`, aguardando o aceite do PI; não há `proplan:next` entre as issues abertas do MVP-025.** Histórico detalhado em `docs/STATUS-ARQUIVO.md`; o board vence divergência factual.
...
| Feito | [#240](https://github.com/RodReis/rrb-jarvisOS/issues/240) · M25-F03 PRD, Landscape e Convention por IA | **entregue** (PR desta fatia). Troca composição por **geração verificada**: origem `brief` como âncora no id da afirmação, síntese sobre evidência com as URLs conferidas contra o conjunto extraído, e contradição como pergunta com recomendação. **Decisão do PI (2026-09-03) invertendo a M8-F04:** falha de pesquisa **não** bloqueia o PRD — só o Landscape fica pendente. **Limite declarado:** o smoke com Claude Code e Tavily reais não foi executado. `proplan:done`, aguardando aceite |
| Feito | [#241](https://github.com/RodReis/rrb-jarvisOS/issues/241) · M25-F04 Arquitetura por IA após os anexos | **entregue** (PR desta fatia). Troca a composição da M8-F05 por **geração verificada**: `prototipo` vira origem de primeira classe com o hash do anexo, e **fluxo sem âncora em protótipo reprova** no validador. O **gate de anexos da M8-F05 permanece intacto e anterior** — nenhuma chamada ao modelo enquanto faltar anexo — e a validação determinística roda antes da IA, que só acrescenta a análise de coerência como proposta. **Decisão do PI (2026-09-03):** a IA passa a ser o único caminho, e as funções compositoras saem junto. **Limite declarado:** o smoke com o Claude Code real não foi executado. `proplan:done`, aguardando aceite |
| Feito | [#242](https://github.com/RodReis/rrb-jarvisOS/issues/242) · M25-F05 Roadmap, MVPs e SPEC por IA | **entregue** (PR desta fatia); **fecha o MVP-025**. Troca a composição da M8-F06 por **geração verificada**: os MVPs nascem do PRD aceito e da arquitetura aprovada com origem por item, e a **IA regenera contra o validador de DAG, que continua sendo a autoridade sobre o grafo**. **Decisões do PI (2026-09-03):** no `MVP_ENTRY` ele escolhe entre os MVPs elegíveis (substituindo o automático da M8-F06); o MVP aceito é **congelado** na regeneração; as perguntas da SPEC têm tabela própria; e a composição sai junto. A SPEC nasce em rascunho com perguntas abertas que deixam o `SLICE_ENTRY` **sem objeto** até serem respondidas. **Limite declarado:** o smoke com o Claude Code real não foi executado. `proplan:done`, aguardando aceite |
...
| MVP-025 Jornada de planejamento por IA | [#237](https://github.com/RodReis/rrb-jarvisOS/issues/237) | criado em 2026-09-03; corrige o fluxo inicial do MVP-008 (prompt, refinamento e geração por IA, jornada única). Cinco SPECs `aprovada-pi`; **M25-F01 a F05 entregues** e aguardando aceite; doc em `docs/mvp/mvp-025-jornada-planejamento-ia.md` | 5/5 |
...
5. **Fila:** logo após o MVP-009 fechar, antes do MVP-022 e do Command Center. **As cinco fatias foram entregues em 2026-09-03; nenhuma issue aberta do MVP-025 carrega `proplan:next`.**
...
1. **MVP-009 finalizado e épico [#100](https://github.com/RodReis/rrb-jarvisOS/issues/100) fechado** pelo PI em 2026-09-03, com as seis fatias-filhas em `proplan:finalizado`. As **cinco fatias do MVP-025 (#238 a #242) foram puxadas e entregues** na mesma data, e o épico **#237 fica com todas as filhas em `proplan:done`, aguardando o aceite do PI** — que é quem fecha as issues e o épico. **A cabeça da fila seguinte é decisão do PI:** MVP-022 e Command Center continuam depois do MVP-025, e nenhuma issue foi promovida a `next` por esta entrega.
...
Base entregue → MVP-009 (finalizado) → **MVP-025 Jornada de planejamento por IA** → Shell MVP-022 → Command Center MVP-017 → MVP-018/MVP-019 → MVP-020 → MVP-021 (decisão do PI de 2026-09-03: o MVP-025 passa na frente do Shell e do Command Center). MVP-010–016 seguem a prioridade já definida pelo PI, sem ultrapassar essa sequência por causa deste PR.
