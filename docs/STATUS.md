...
> **`proplan:next` sem sucessor definido.** O `[FIX]` **#41** (renderer em porta variável × `strictPort`) saiu para entrega em 2026-07-24 (PR #73), confirmado como frente atual pelo PI. Restam na fila-cabeça (decisão do PI, item 11 abaixo): o `[FIX]` **#64** (rótulo do login quebra o botão) ou o **MVP-004** ([#10](https://github.com/RodReis/rrb-jarvisOS/issues/10)). Enquanto o PI não marca, nenhum card carrega `proplan:next`.
...
> Card **[#41](https://github.com/RodReis/rrb-jarvisOS/issues/41)** (`[FIX]`, 2026-07-24): **o renderer subia em porta variável** — a base era 5173 (default do Vite), não 5180, e sem `strictPort` ele saltava de porta até achar uma livre (log real: 5173→5177), contra `CLAUDE.md` § Portas. Porta que "se conserta sozinha" faz abrir a tela do processo errado sem perceber. Corrigido no `renderer` de `electron.vite.config.ts`: `server: { host: '127.0.0.1', port: 5180, strictPort: true }` — mesmo par do precedente `vite.prova.config.ts` (o `127.0.0.1` explícito evita a resolução IPv6 do `localhost` no Windows). Provado no app real: `electron-vite dev` sobe em `http://127.0.0.1:5180/`; com a 5180 ocupada, `Error: Port 5180 is already in use` — falha ruidosa, sem saltar. **Mergeado em 2026-07-24** (PR [#73](https://github.com/RodReis/rrb-jarvisOS/pull/73)), aguardando aceite.
...
| [#41](https://github.com/RodReis/rrb-jarvisOS/issues/41) | — (`[FIX]`) | MVP-001 | *(sem spec — fonte: CLAUDE.md § Portas, `vite.prova.config.ts`)* | — | [#73](https://github.com/RodReis/rrb-jarvisOS/pull/73) | 2026-07-24 |
...
11. **PI — decidir a próxima frente**: o `[FIX]` **#64** (rótulo do login quebra o botão) ou o **MVP-004** ([#10](https://github.com/RodReis/rrb-jarvisOS/issues/10), execução real + terminal). O `[FIX]` **#41** (renderer × `strictPort`) foi entregue em 2026-07-24 (PR #73).
...
14. ~~**PI — decidir sobre a tela CHOICE**~~ — **decidido** (2026-07-24). Os 4 bloqueadores e os 4 residuais estão resolvidos e registrados na **`docs/spec/spec-choice-01-selecao-de-espaco.md`**: a CHOICE entra em **todo login**, é a **porta única** de troca de espaço (o rail deixa de alternar), o acento persiste no `UserProfile` **por usuário e sem `AuditEvent`**, a **tela de transição entra nesta fatia** (decisão do PI contrária à proposta do Cowork, que sugeria adiar) e a fatia vem **antes do MVP-004**, com o container de MVP **em aberto**. Falta o **Cowork**: marcar a spec `aprovada-pi`, fazer a emenda à SPEC-Fundacao-02 (a spec revoga comportamento entregue e aceito no PR #28) e criar a issue-fatia. **O Code não implementa antes disso.** Duas notas registradas na spec: sem MVP pai a issue nasce como card solto no board, e com a transição dentro a fatia acumula quatro frentes — se não fechar em uma entrega, o Code para e reporta em vez de fatiar por conta própria.
