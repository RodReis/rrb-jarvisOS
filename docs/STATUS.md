...
|---|---|---|---|---|
| [#84](https://github.com/RodReis/rrb-jarvisOS/issues/84) | `[FIX]` card de aprovação descreve comando como filesystem | MVP-004 (#10) | *(sem spec — fonte: SPEC-DS-04b §21 + contrato `AcaoSensivel`)* | — |
...
>
> ✅ **Ressalva fechada em 2026-08-28 — a UI foi verificada no app real.** O login voltou a funcionar (duas causas de configuração: o projeto Supabase estava **pausado** e `127.0.0.1:*` não estava na allowlist de Redirect URLs; nenhuma linha de código mudou), e com sessão real as rotas `operacoes` e `terminal` ficaram alcançáveis pela primeira vez. Os 8 critérios de comportamento confirmados no app, com efeito no disco; `verifyAuditChain` → `{ok: true, checked: 77}`. **A fila de aprovações renderiza** — fecha também a ressalva que a F01 tinha deixado aberta. Capturas em `reports/comparacao/f01-fila-aprovacoes-com-card.png`, `f02-terminal-inicial.png`, `f02-e-executou.png`, `f02-g-aguardando-aprovacao.png`.
>
> A verificação achou **dois defeitos que a suíte verde não pegaria** — a mesma lição de método do MVP-003 se repetindo: o card de aprovação descreve comando de terminal como filesystem (**#84**), e a allowlist de **diretórios** não tem UI nenhuma (pendência de fatia, ver Próximas ações).
...
1. ~~**PI — decidir a ressalva aberta da F01.**~~ **Resolvida em 2026-08-28.** O login estava quebrado por **duas causas de configuração**, não por código: o projeto Supabase `puifraayzsqdqbsoqsez` estava **pausado** (`INACTIVE`), e `http://127.0.0.1:*` não estava na allowlist de **Redirect URLs** do Supabase — o callback caía no Site URL e o servidor loopback nunca recebia o código (o log parava em *"Fluxo de login aberto no navegador"* e morria no timeout de 5 min). O console do Google estava correto o tempo todo: ele só devolve para o Supabase, nunca para o app. Com o login funcionando, a fila de aprovações e o terminal foram verificados no app real — ver a nota em **Feito**.

1b. **Cowork/PI — fatia para a UI da allowlist de diretórios.** A verificação expôs que os canais `allowlist:list/add/remove` existem na ponte desde o MVP-002, mas **nenhuma tela os usa** (zero referência em `src/renderer`). Na prática o usuário **não consegue** permitir um diretório pelo app — e sem isso o terminal da F02 não executa nada, porque o cwd sempre cai fora. Não é `[FIX]`: não há parágrafo definindo onde essa tela mora, se usa seletor de pasta nativo ou se o `appDir` é removível. É decisão de produto ⇒ spec + aval do PI.
