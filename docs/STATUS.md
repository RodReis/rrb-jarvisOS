...
| Backlog | [#259](https://github.com/RodReis/rrb-jarvisOS/issues/259) · [FIX] Aceite documental é desfeito pelo recálculo | Achado na M26-F01. `brief-aceito` e `prd-aceito` não deixam evidência que `eventosObservados` enxergue, então `etapaDerivada` para em `prd-aceito` e **nenhum projeto real alcança Especificação nem Construção**. Comportamento correto na SPEC-Jornada-01, critérios 1 e 2 — correção de bug documentado, sem spec nova |
