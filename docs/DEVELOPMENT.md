...
Status: **entregue** — spec `aprovada-pi` (2026-07-21); issue [#15](https://github.com/RodReis/rrb-jarvisOS/issues/15); PR [#31](https://github.com/RodReis/rrb-jarvisOS/pull/31). **Ambiente, roda independente.** Só o alvo de sync — o sync em si é Corte 3.
...
- [x] Entrega: PR [#31](https://github.com/RodReis/rrb-jarvisOS/pull/31) (`refs #15`); docs/ commitados
...
6. **`(select auth.uid())` e não `auth.uid()` nas policies** — com o select, o planner avalia uma vez por consulta em vez de uma por linha. Diferença invisível no seed de dois usuários, relevante quando o espelho tiver volume.
