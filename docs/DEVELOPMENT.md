...
7. **Achado de processo: gerar o relatório com a stack Supabase parada envenena o número.** Custou uma rodada de CI vermelha nesta fatia. Com o Docker desligado, `npm run test:report` roda até o fim e escreve `Banco 110 pass` em vez de `117` — os 7 testes de RLS se pulam (comportamento correto: não punir quem clona sem Docker) e simplesmente somem da contagem. Nada falha, nada avisa: o arquivo parece válido. Quem pegou foi a **guarda anti-drift** do ADR-003, comparando o commitado com uma execução limpa — exatamente o que ela existe para fazer, e a prova de que a rotina de relatório não é decorativa. Registrado em `docs/TESTING.md` §7 como passo obrigatório: conferir `npx supabase status` antes de gerar o relatório.
