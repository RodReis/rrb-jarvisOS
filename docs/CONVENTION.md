...
Integração do Graphify aprovada: adapter substituível sob contrato próprio, com compatibilidade verificada e entrada limitada às fontes selecionadas por identidade/revisão. Não amplia acessos nem instala configurações globais automaticamente. Extração estrutural local de código; semântica pelos executores autorizados, dentro do orçamento existente, sem migração silenciosa para API paga. Saída distingue extração/inferência e preserva fonte/revisão; relações afetadas por correções/remoções deixam de valer como atuais. Reflexões, quando usadas, são candidatas com proveniência, sem substituir decisões do PI ou promover políticas automaticamente. Ausência/incompatibilidade mantém busca básica; atualizações exigem testes de compatibilidade, sem atualização silenciosa. Detalhes técnicos permanecem abertos conforme a seção 12 do design.
