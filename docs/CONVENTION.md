...
Contrato proposto pela M7-F06, ainda sujeito ao aceite da revisão exata: lição operacional usa identidade `moduleId + lessonKey + projectId`, revisões materiais e ledger imutável de evidências/avaliações. Estados públicos são `candidate`, `validated` e `needs_revalidation`; não existe validação parcial pública nem generalização automática entre projetos. O módulo de origem define critérios e dimensões materiais versionados. Aprovação do PI comprova decisão, não resultado empírico; PR/CI/merge comprovam gates, não impacto. Falta de cobertura é explícita, falha técnica não vira avaliação semântica e perda da prova retira a validação vigente sem ressuscitar conteúdo excluído. Fonte: `docs/spec/spec-memoria-06-validacao-licoes-operacionais.md`.

Validação das lições fora da pipeline aprovada: candidata rastreável contém afirmação, contexto, projeto, fontes e resultados; repetição não cria evidência. O módulo responsável valida por critérios verificáveis definidos em sua especificação; a memória registra sem inventar sucesso. Critérios objetivos permitem automação sem aceite duplicado; ausência de critério/evidência mantém candidata sem bloquear desenvolvimento. Validade limitada a condições/versões comprovadas, incluindo falhas e contrapontos; sucesso isolado não vira regra universal. Mudança relevante ou contradição retira a validade vigente até reavaliação, preservando histórico. Orientação do PI é decisão, não prova empírica; lições não concedem novas permissões e a pipeline mantém o MVP-016. O contrato concreto inicial está em revisão na M7-F06 conforme a seção 19 do design.
