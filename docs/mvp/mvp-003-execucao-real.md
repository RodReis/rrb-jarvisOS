# MVP-003 — Execução real (terminal + execução allowlisted)

- Tipo: épico (`proplan:mvp`). Container de fatias — **sem spec própria**.
- Status: **definido** (2026-07-21) — 2ª metade do Corte 2 (split aprovado pelo PI). Épico criado ([#10](https://github.com/RodReis/rrb-jarvisOS/issues/10)); fatias lazy. **BudgetPolicy movida para o MVP de providers (Corte 3)** — decisão do PI 2026-07-21.
- Base: `docs/iniciais/requisitos-agent-os.md` § Corte 2; ADR-001 (local-first), ADR-004 (auditoria), ADR-005 (logging).
- Depende de: **MVP-002 entregue** (Policy Engine, allowlist, execução simulada auditada). Sem essa base, não há guardrail para ligar a execução real.
