...
> **Renumerado de MVP-003 → MVP-004 em 2026-07-21** (decisão do PI): o slot MVP-003 passou a ser o **Design System da Plataforma** (`docs/mvp/mvp-003-design-system-plataforma.md`). O conteúdo, a tese e as fatias abaixo são os mesmos do antigo MVP-003; muda só o número.

- Tipo: épico (`proplan:mvp`). Container de fatias — **sem spec própria**.
- Status: **finalizado e aceito pelo PI** (2026-08-31). Épico [#10](https://github.com/RodReis/rrb-jarvisOS/issues/10) fechado com três fatias entregues. **BudgetPolicy movida para o MVP de providers (Corte 3)** — decisão do PI 2026-07-21.
...
As três fatias foram entregues e aceitas. A fronteira F01×F02 foi cravada pelo PI (2026-07-24): **F01 = só filesystem; comando/processo é exclusivo da F02.**

- [x] **Fatia 01 — Execução real de filesystem allowlisted (enforcement + aprovação):** issue [#74](https://github.com/RodReis/rrb-jarvisOS/issues/74). Spec: `spec-execucao-real-01-filesystem-allowlisted.md`.
- [x] **Fatia 02 — Terminal controlado no Desktop (command-runner):** issue [#75](https://github.com/RodReis/rrb-jarvisOS/issues/75). Spec: `spec-execucao-real-02-terminal-controlado.md`.
- [x] **Fatia 03 — UI da allowlist de diretórios em Settings:** issue [#110](https://github.com/RodReis/rrb-jarvisOS/issues/110). Spec: `spec-execucao-real-03-ui-allowlist-diretorios.md`.
