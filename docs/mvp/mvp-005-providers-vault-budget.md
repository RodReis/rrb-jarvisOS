...
- [ ] **Fatia 04 — Multi-provider + primeiro executor (`ProviderRoute`):** Gemini + Ollama usam o runtime HTTP; **Claude Code CLI** inaugura o runtime irmão `CodingExecutorAdapter`, compartilhando Vault/auditoria/ledger/health sem fingir ser chamada HTTP. Roteamento por tarefa, fallback, healthcheck e tela de providers/executores. A assinatura é `subscription_limited`; Codex entra somente no MVP-010. **Spec `aprovada-pi`:** `spec-providers-04-multi-provider-roteamento.md`.
...
- Roteamento por tarefa seleciona alvo discriminado `provider | executor` e chama o runtime compatível; a tela mostra status/latência/modelo sem expor segredo.
