...
> **Split do Corte 3 em três MVPs (2026-07-24).** Por decisão do PI, "Integrações reais" foi partido em **MVP-005 Providers + Vault + BudgetPolicy** (`docs/mvp/mvp-005-providers-vault-budget.md`), **MVP-006 Conectores externos** e **MVP-007 Memória híbrida + RAG** — mesma lógica do split do Corte 2 (não concentrar risco). O **sync bidirecional local↔Supabase fica diferido**: a estratégia de conflito multi-dispositivo é questão aberta do ADR-001 (q3); o MVP-007 especifica a memória **local** primeiro. Só o MVP-005 está definido no board até aqui.
