...
- O código (gerador, orquestrador, config, CI, `reports/`) é entregue **na Fatia 01 (Bootstrap)**, junto do passo "scripts dev/build/lint/test configurados" — não há `package.json`/runners antes disso. Instalá-lo antes furaria o gate WIP=1 do processo.
...
- Playwright-Electron no CI Linux exige `xvfb-run` (o app abre janela).
