...
- Status: **aprovada-pi** (2026-07-21) — decisões de biblioteca, retenção e escopo resolvidas pelo PI.
...
- **Categorias** (child logger por categoria), com campo `direction: 'in' | 'out'` onde houver fluxo:
  ...
  - **Nota de escopo:** `ai`, `agent` e `integracao` **não têm fluxo na fundação** (estão fora do MVP-001). Aqui define-se o **contrato** dessas categorias (assinatura, formato, redaction) para que, quando os subsistemas existirem, o log de in/out já esteja padronizado. O que loga de verdade agora: `auth` (SPEC-03), `db` (SPEC-04), `ipc`, `sistema` e a troca de workspace (SPEC-02).
...
5. A regra "todo método loga" está aplicada aos fluxos existentes da fundação (auth, workspace-switch, db, ipc) — cada um emite `info`/`warn`/`error` conforme o caso.
