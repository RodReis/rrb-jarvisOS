...
- **Emenda 2026-08-29 (PI, na revisão do MVP-008):** o gate de USD vale **somente para rota paga** (Claude API/BYOK). A **rota de assinatura** — plano Claude MAX pelo Claude Code CLI (M5-F04) — **não tem custo por chamada**: o ledger registra uso (chamadas, tokens, tempo) **sem valor monetário** e a `BudgetPolicy` **não a barra**. Converter uso de assinatura em USD estimado foi **descartado**: seria número inventado, e o gate bloquearia com base nele. Créditos de conector seguem no ledger próprio da SPEC-Conectores-02.
