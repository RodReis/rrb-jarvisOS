...
- **Rotação/expiração automática de segredo** — **fora**: adicionar/editar/remover basta nesta fatia; rotação é futuro. **Emenda 2026-08-29 (PI):** o "futuro" tem dono — o suporte a **payload cifrado estruturado** (access token + refresh token + `expires_at`), a `expires_at` como **metadado consultável sem decifrar** e à **rotação atômica** é escopo da **M6-F03** (`spec-conectores-03-github-app-autenticacao.md`), exigido pelo OAuth Device Flow do GitHub. A M5-F01 (#77) **não é reaberta**.
