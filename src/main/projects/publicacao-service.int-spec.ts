...
const { chaveDeFatia, chaveDeMvp } = await import('@shared/domain/publicacao')
...
comGit('publicação do repositório e do backlog', () => {
  it('publica repositório, commits e issues do MVP e das fatias', async () => {
    ...
    // Um MVP e duas fatias: três issues, e a issue da fatia não existe sem a do MVP.
    expect(operacoes(GITHUB_OPERATIONS.ensureIssue)).toHaveLength(3)
    expect(operacoes(GITHUB_OPERATIONS.ensureIssueDependency)).toHaveLength(2)
  })
  ...
  it('as issues carregam a chave determinística do par MVP/Fatia (critério 3)', async () => {
    ...
    expect(chaves).toContain(chaveDeFatia(PROJETO, 1, 2))
  })
  ...
  it('repetir NÃO duplica: a segunda publicação não cria nada (critério 1)', async () => {
    ...
    expect(issuesPorChave.size).toBe(3)
  })
...
comGit('bloqueio retomável (critério 5) e reconciliação (critério 6)', () => {
  ...
  it('o bloqueio no meio não desfaz o que já foi publicado — nem repete na retomada', async () => {
    ...
    // O que saiu antes do bloqueio continua lá: a issue do MVP e a da primeira fatia.
    ...
    expect(issuesPorChave.size).toBe(3)
    // Uma criação só — a fatia que faltava. Se a reconciliação falhasse, seriam três.
    expect(segunda.criados).toBe(1)
  })
