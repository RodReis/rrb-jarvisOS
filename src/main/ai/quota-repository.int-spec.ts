...
describe('QuotaRepository', () => {
  ...
  it('gravar de novo sobrescreve, nunca duplica linha', () => {
    ...
    repo.gravar(
      'u1',
      'ws1' as never,
      { provider: 'claude-code', origem: 'desconhecida', atualizadoEm: t1.toISOString() },
      t1
    )
    repo.gravar(
      'u1',
      'ws1' as never,
      { provider: 'claude-code', origem: 'medida', restante: 5, atualizadoEm: t2.toISOString() },
      t2
    )
    ...
    const contagem = db.prepare('SELECT COUNT(*) as n FROM provider_quota_state').get() as {
      n: number
    }
  ...
  it('escopos distintos (workspace, provider) não colidem', () => {
    ...
    repo.gravar(
      'u1',
      'ws1' as never,
      { provider: 'claude-code', origem: 'medida', restante: 1, atualizadoEm: agora.toISOString() },
      agora
    )
    repo.gravar(
      'u1',
      'ws2' as never,
      { provider: 'claude-code', origem: 'medida', restante: 2, atualizadoEm: agora.toISOString() },
      agora
    )
